...
wilcox.test(
  releases_committers_bproj$time_precision.few,
...
wilcox.test(
  releases_committers_bproj$time_recall.few,
...
wilcox.test(
  releases_committers_bproj$range_precision.few,
...
releases_trad_bproj <- releases_wcy %>% 
  filter(cycle_sec > cycle_treshold) %>%
  group_by(project) %>% 
  summarise(time_precision = mean(time_precision),
            time_recall = mean(time_recall),
...
releases_speed_bproj <- releases_rapid_bproj %>% 
  inner_join(releases_trad_bproj, by = "project",
      suffix = c(".few", ".many"))

releases_speed_bproj_melted <- releases_speed_bproj %>% 
  select(-releases.few, -releases.many) %>%
  gather(variable, value, -project) %>%
  mutate(
    group = case_when(grepl("few", variable) ~ "few", TRUE ~ "many"),
...
wilcox.test(
  releases_speed_bproj$time_precision.few,
  releases_speed_bproj$time_precision.many,
  paired = TRUE)
cliff.delta(
  releases_speed_bproj$time_precision.few,
  releases_speed_bproj$time_precision.many)
...
wilcox.test(
  releases_speed_bproj$time_recall.few,
  releases_speed_bproj$time_recall.many,
  paired = TRUE)
cliff.delta(
  releases_speed_bproj$time_recall.few,
  releases_speed_bproj$time_recall.many)
...
wilcox.test(
  releases_speed_bproj$range_precision.few,
  releases_speed_bproj$range_precision.many,
  paired = TRUE)
cliff.delta(
  releases_speed_bproj$range_precision.few,
  releases_speed_bproj$range_precision.many)

releases_speed_bproj_melted %>%
  filter(
    variable == "time_precision.few" | variable == "time_precision.many" |
    variable == "range_precision.few" | variable == "range_precision.many"
  ) %>%
  ggplot(aes(x=variable, y=value)) +
    ggtitle("Precision - rapid vs traditional releases") +
    geom_boxplot() +
    coord_flip() +
    scale_x_discrete(labels=c(
      "time_precision.many" = "many",
...
releases_speed_bproj_melted %>%
  filter(variable == "time_recall.few" | variable == "time_recall.many") %>%
  ggplot(aes(x=variable, y=value)) +
    ggtitle("Recall - rapid vs traditional releases") +
    geom_boxplot() +
    coord_flip() +
    scale_x_discrete(labels=c(
      "time_recall.many" = "many",
